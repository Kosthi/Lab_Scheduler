...
//    public boolean getUser(String url) {
...
//    }
    ...
    public static String postJson(String url, Map<String, Object> params) {
        try {
            ...
            if (response.isSuccessful() && response.body() != null) {
                String result = response.body().string();
                logger.info("执行POST请求, URL: {} 成功，返回数据: {}", url, result);
                return result;
            } else {
                logger.warn("POST请求未成功, URL: {}, 响应码: {}", url, response.code());
            }
        } catch (Exception e) {
            logger.error("执行POST请求，URL: {} 出现异常!", url, e);
        }
        return null;
    }
